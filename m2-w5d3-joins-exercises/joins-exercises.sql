...
SELECT CONCAT(c.first_name, ' ', c.last_name) AS cust_name, COUNT(*) as rentals
FROM customer c
...
SELECT CONCAT(c.first_name, ' ', c.last_name) AS cust_name, SUM(pa.amount) as rentals
FROM customer c
JOIN rental r ON r.customer_id = c.customer_id
...
SELECT s.store_id, a.address, COUNT(r.rental_id) AS rentals, SUM(p.amount) AS sales, (SUM(p.amount) / COUNT(r.rental_id)) AS avg_sale
FROM store s
JOIN address a ON a.address_id = s.address_id
...
SELECT f.title, COUNT(r.rental_id) AS tot_rental
FROM rental r
JOIN inventory i ON i.inventory_id = r.inventory_id
JOIN film f ON f.film_id = i.film_id
GROUP BY f.title
ORDER BY tot_rental DESC LIMIT 10

-- 17. The top five film categories by number of rentals 
-- (#1 should be “Sports” with 1179 rentals and #5 should be “Family” with 1096 rentals)

SELECT ca.name, COUNT(r.rental_id) AS rentals
FROM film f
JOIN film_category fc ON fc.film_id = f.film_id
...
SELECT CONCAT(a.first_name, ' ', a.last_name) AS actor_name, COUNT(r.rental_id) AS rentals
FROM film f
JOIN film_actor fa ON fa.film_id = f.film_id
JOIN actor a ON a.actor_id = fa.actor_id
JOIN inventory i ON i.film_id = f.film_id
